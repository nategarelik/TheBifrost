...
The Bifrost 是 Model Context Protocol 在 Unity 编辑器中的个性化实现，由 Nathaniel Garelik 设计，专为私人使用而设计，允许 AI 助手与您的 Unity 项目交互。这个包提供了 Unity 和实现 MCP 协议的 Node.js 服务器之间的桥梁，使 AI 代理能够在 Unity 编辑器中执行操作。

## 功能

### IDE 集成 - 包缓存访问

The Bifrost 通过将 Unity `Library/PackedCache` 文件夹添加到您的工作区，提供与 VSCode 类 IDE（Visual Studio Code、Cursor、Windsurf）的自动集成。此功能：
...
### 步骤 1: 通过 Unity 包管理器安装 The Bifrost 包
1. 打开 Unity 包管理器 (Window > Package Manager)
2. 点击左上角的 "+" 按钮
3. 选择 "Add package from git URL..."
4. 输入: `[您的 The Bifrost 私人 Git URL]` (如果是本地包，则从磁盘添加)
5. 点击 "Add"

<!-- 移除包管理器图片 -->

### 步骤 2: 安装 Node.js
> 要运行 The Bifrost 服务器，您需要在计算机上安装 Node.js 18 或更高版本：
...
> 将 `ABSOLUTE/PATH/TO` 替换为您的 The Bifrost 安装的绝对路径，或者直接从 Unity 编辑器 MCP 服务器窗口（Tools > TheBifrost > Server Window）复制文本。
...
The Bifrost 服务器有两种方式：

### 选项 1: 通过 Unity 编辑器启动
1. 打开 Unity 编辑器
2. 导航到 Tools > TheBifrost > Server Window
3. 点击 "Start Server" 按钮

### 选项 2: 通过命令行启动
1. 打开终端或命令提示符
2. 导航到 The Bifrost 服务器目录
3. 运行以下命令：
   ```bash
...
1. 打开 Unity 编辑器
2. 导航至 Tools > TheBifrost > Server Window
...
- 确认端口号正确（默认是 8090）
- 可在 Unity 编辑器 The Bifrost Server 窗口更改端口号。(工具 > TheBifrost > Server Window)
</details>
